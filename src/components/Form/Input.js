	...
	return (
		<div className="input">
			...
			{props.object.type === "text-group" && (
				<div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '20px', width: '100%' }}>
					{props.object.questions.map((question, i) => (
						<div key={i} style={{ minWidth: '200px', flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
							{question.type === "text" && (
								<TextField
									fullWidth
									required
			...
			{props.object.type === "multi-radio" && (
				<>
					<h2>{props.object.label}</h2>
					<FormGroup row sx={{ justifyContent: 'center', flexWrap: 'wrap', gap: 1 }}>
						{props.object.options.map((option, i) => (
							<FormControlLabel
								key={i}
								control={
